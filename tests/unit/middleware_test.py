...
class TestMiddleware(Middleware):
    ...
    async def local_action(self, next_handler, action):
        ...
        async def wrapped_handler(ctx):
            ...
            if isinstance(res, dict) or hasattr(res, '__setitem__'): # Check if it can be a dict or supports item assignment
...
@pytest.fixture
async def broker_instance(): # Removed loop, pytest-asyncio handles it
    broker = Broker(id="test-broker-mw")
    # yield broker # Proper fixture with teardown
    # await broker.stop() 
    return broker # Simpler for now

def test_broker_registers_middlewares_direct():
    mw1 = TestMiddleware(name="MW1")
    broker = Broker(id="direct-broker", middlewares=[mw1])
...
def test_broker_registers_middlewares_via_settings():
    ...
    broker = Broker(id="settings-broker", settings=settings)
    assert len(broker.middlewares) == 1
    assert broker.middlewares[0] == mw1
    assert len(mw1.called_hooks) == 1
    assert mw1.called_hooks[0]["hook"] == "broker_created"
...
@pytest.mark.asyncio
async def test_local_action_single_middleware(loop): # loop fixture might not be needed if using pytest-asyncio defaults
    mw1 = TestMiddleware(name="MW1")
...
@pytest.mark.asyncio
async def test_local_action_multiple_middlewares_chaining(loop):
...
@pytest.mark.asyncio
async def test_remote_action_single_middleware(loop):
    ...
    # Use the Action alias from 'from pylecular.registry import ActionEndpoint as Action'
    action_endpoint = Action(node_id="remote-node", service=SimpleService(), method=None, name=remote_action_name, is_local=False)
    ...
    with patch.object(broker.transit, 'request', new_callable=AsyncMock) as mock_transit_request:
        mock_transit_request.return_value = {"data_from": "remote_mock", "original_params": ctx_params.copy()}
...
@pytest.mark.asyncio
async def test_remote_action_multiple_middlewares_chaining(loop):
    mw1 = TestMiddleware(name="MW1_Remote") 
    ...
    action_endpoint = Action(node_id="remote-node-c", service=SimpleService(), method=None, name=remote_action_name, is_local=False)
    broker.registry.add_action(action_endpoint)
    ...
    with patch.object(broker.transit, 'request', new_callable=AsyncMock) as mock_transit_request:
        mock_transit_request.return_value = {"data_from": "remote_chain_mock", "original_params": ctx_params.copy()}
...
@pytest.mark.asyncio
async def test_local_event_emit_single_middleware(loop):
    ...
    assert hook_call["event_name"] == "emitter.user_created" # Check based on TestMiddleware recording
...
@pytest.mark.asyncio
async def test_local_event_emit_multiple_middlewares_chaining(loop):
    ...
    mw1.reset(); mw2.reset()
    await broker.start()
    mw1.called_hooks.clear(); mw2.called_hooks.clear()
...
@pytest.mark.asyncio
async def test_local_event_broadcast_multiple_handlers_and_middlewares(loop):
...
@pytest.mark.asyncio
async def test_broker_created_hook(loop):
...
@pytest.mark.asyncio
async def test_broker_started_hook(loop):
...
@pytest.mark.asyncio
async def test_broker_stopped_hook(loop):
...
@pytest.mark.asyncio
async def test_service_created_and_started_hooks(loop):
...
# If Action/Event objects passed to middleware hooks are simple dicts or other types, adjust TestMiddleware accordingly.
