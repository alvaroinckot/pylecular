...
from pylecular.settings import Settings # Included for completeness, though direct middleware passing is used
...
class LoggingMiddleware(Middleware):
    ...
    async def local_action(self, next_handler, action_endpoint):
        ...
        async def wrapped_handler(ctx):
            ...
            print(f"[LOGGING_MW] ---> Calling action '{action_endpoint.name}' | Params: {ctx.params} | Meta: {ctx.meta}")
    ...
    async def local_event(self, next_handler, event_endpoint):
        ...
        async def wrapped_handler(ctx):
            ...
            print(f"[LOGGING_MW] ---> Emitting event '{event_endpoint.name}' | Params: {ctx.params} | Meta: {ctx.meta}")
...
class EnrichContextMiddleware(Middleware):
    ...
    """
    async def local_action(self, next_handler, action_endpoint):
        print(f"[ENRICH_MW] '{action_endpoint.name}' action is being prepared with context enrichment.")

        async def wrapped_handler(ctx):
            timestamp = time.time()
            ctx.meta['request_timestamp'] = timestamp
            print(f"[ENRICH_MW] Added 'request_timestamp' ({timestamp:.6f}) to meta for action '{action_endpoint.name}'.")
...
class GreeterService(Service):
    ...
    async def say_hello(self, ctx):
        ...
        print(f"[SERVICE_ACTION] GreeterService.say_hello: Processing request. Meta received: {ctx.meta}")
    ...
    async def item_updated(self, ctx):
        ...
        print(f"[SERVICE_EVENT] GreeterService.item_updated: Item '{item_id}' was updated. Params: {ctx.params}, Meta: {ctx.meta}")
...
async def main():
    ...
    broker = Broker(
        id="eg-broker-mw", 
        middlewares=[logging_mw, enrich_mw]
    )
    print(f"Broker '{broker.id}' created with middlewares: {[mw.__class__.__name__ for mw in broker.middlewares]}")
