...
class Broker:
    ...
    async def start(self):
        ...
        # Call broker_started hooks
        await asyncio.gather(*[
            middleware.broker_started(self)
            for middleware in self.middlewares
            if hasattr(middleware, 'broker_started') and callable(getattr(middleware, 'broker_started'))
        ])
    ...
    async def stop(self):
        ...
        # Call broker_stopped hooks
        await asyncio.gather(*[
            middleware.broker_stopped(self)
            for middleware in self.middlewares
            if hasattr(middleware, 'broker_stopped') and callable(getattr(middleware, 'broker_stopped'))
        ])
    ...
    async def register(self, service): # Make it async
        ...
        # Call service_created hooks
        await asyncio.gather(*[
            middleware.service_created(service)
            for middleware in self.middlewares
            if hasattr(middleware, 'service_created') and callable(getattr(middleware, 'service_created'))
        ])

        # Call service_started hooks (called immediately after created for now)
        await asyncio.gather(*[
            middleware.service_started(service)
            for middleware in self.middlewares
            if hasattr(middleware, 'service_started') and callable(getattr(middleware, 'service_started'))
        ])

        # TODO: Implement service_stopped hook, likely in a new Broker.unregister method or similar

    def _apply_middlewares(self, handler, hook_name, metadata):
        for middleware in reversed(self.middlewares):
            hook = getattr(middleware, hook_name, None)
            if hook:
                handler = hook(handler, metadata)
        return handler
        ...
        if endpoint and endpoint.is_local:
            ...
            try:
                ...
                if handler_to_execute is None: # Check the potentially wrapped handler
                    raise Exception(f"Handler for action {action_name} is None after applying middlewares.")
            ...
            wrapped_remote_request_handler = self._apply_middlewares(
    ...
    async def emit(self, event_name, params={}, meta={}):
        ...
        if endpoint and endpoint.is_local and endpoint.handler:
            handler_to_execute = self._apply_middlewares(endpoint.handler, "local_event", endpoint)
    ...
    async def broadcast(self, event_name, params={}, meta={}):
        ...
        await asyncio.gather(
            *[
                (await self._apply_middlewares(endpoint.handler, "local_event", endpoint)(context))
